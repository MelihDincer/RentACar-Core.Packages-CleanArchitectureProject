    ...
    public static string Transform(Filter filter, IList<Filter> filters)
    {
        ...
        if (!string.IsNullOrEmpty(filter.Value))
        {
            if (filter.Operator == "doesnotcontain")
                where.Append($"(!np({filter.Field}).{comparison}(@{index.ToString()}))");//sen bana bir string ver benim kurallarıma standartlarıma göre. Ben de onu sana linq'e çevireyim diyor burada.
